...
app.get('/gr',function(req,res){
  ...
  map.forEach(function(person,key){
      var template = "";
      var sno = 1;
    var consumer_name="";   
    ...
    result = result.replace("{{consumer-bill-amount}}",total.toFixed(2));
      ...
      //console.log(result) 
      fs.writeFile('./tmp/invoice-template-'+key+'.html',result,'utf8',function(err,data){
        var html3 = fs.readFile('./tmp/invoice-template-'+key+'.html','utf8',function(err,data){
      //console.log(data);
      console.log("converting to pdf "+ key + person+"\n");
       pdf.create(data, options).toFile('./uploads/invoices/invoice_'+key+'_'+consumer_name+'_'+date+'.pdf', function(err, res) {
